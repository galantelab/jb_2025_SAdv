...
##   - 'differential transcript /gene expression' (DTG) with 'Kallisto / Sleuth / TXimport | DESeq2';
##   - 'differential alternative splicing' (DAS) with 'Kallisto / SUPPA2'.
...
# My 'OBJs'
WD=$(pwd)
DATASET="Neandertais"  # NASA | 'Neandertais'
GROUP="Nina"  # test | livia --> Only test groups (LL1-4) | the rest do with Livia | 'Nina'
GENCODE="v36"  # v44 (LATEST) | 'v36 (TCGA / GTEx)' | v37 (PhD)
LOGS=${WD}/logs
TMPDIR=${WD}/myTMPdir
MACHINE=${HOSTNAME}
MY_PID=$$
...
display_usage()
{
        USAGE="${BOLD_BLUE}Usage:${NC} ${BOLD_GREEN}$0${NC} ${BOLD_ORANGE}[OPTIONS]${NC} ${GREY}-->${NC} Pipeline for DEG/DET/AS analyses"
        echo -e "${GREY}===========================================================================================================================================================${NC}\n${USAGE}\n"
        echo -e "This shell script (${ITALIC_GREEN}${VERSION}${NC}) runs automatically the Kallisto tool and others --> user ${ITALIC_GREEN}'${MYUSER}'${NC}.\n"
...
# 'Prints' to time file
echo -e "\n${GREY}==================================================${NC} ${BLUE}${TIMELOG} (${MYUSER})${NC} ${GREY}==================================================${NC}" |& tee -a ${TIME_FILE}
...
echo -e "\t${ORANGE}DATASET:${NC} ${DATASET}" |& tee -a ${TIME_FILE}
echo -e "\t${ORANGE}GROUP:${NC} ${GROUP^^}" |& tee -a ${TIME_FILE}
...
select_script()
{
	...
	#run_script "st0_Metrics.sh" "0.${DATASET}.${GROUP}" "5s" ${DATASET} ${GROUP}  # Runs QC with 'FastQC (v0.11.9) / MultiQC (v1.17)' and, sometimes, 'cutadapt (v3.5)'
	run_script "st1_Kallisto.sh" "1.${DATASET}.${GROUP}" "30s" ${DATASET} ${GROUP} ${GENCODE}  # Pseudo-aligns with 'Kallisto (v0.46.2)', 'GENCODE v36 (TCGA / GTEx)', and 'HG38 (GRCh38.p13)'
}
...
run_script()
{
	...
	echo -e "Executing source step ${ITALIC_BLUE}'$1'${NC}...\n" |& tee -a ${TIME_FILE}
	log_script $2
